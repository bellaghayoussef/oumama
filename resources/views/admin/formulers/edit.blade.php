...
@section('title', 'Modifier le Formulaire')
@section('header', 'Modifier le Formulaire')

@section('content')
<div class="bg-white rounded-lg shadow-md p-6">
    <form action="{{ route('admin.formulers.update', $formuler) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="space-y-6">
            <div>
            ...
            <div>
                ...
                <select name="task_id"
                    ...
                    @foreach($tasks as $task)
                        <option value="{{ $task->id }}" {{ old('task_id', $formuler->task_id) == $task->id ? 'selected' : '' }}>
