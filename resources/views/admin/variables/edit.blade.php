...
@section('title', 'Modifier la Variable')
@section('header', 'Modifier la Variable')
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <select name="type"
                    ...
                    <option value="string" {{ old('type', $variable->type) == 'string' ? 'selected' : '' }}>Texte</option>
                    <option value="bool" {{ old('type', $variable->type) == 'bool' ? 'selected' : '' }}>Booléen</option>
                    <option value="number" {{ old('type', $variable->type) == 'number' ? 'selected' : '' }}>Nombre</option>
                    <option value="file" {{ old('type', $variable->type) == 'file' ? 'selected' : '' }}>Fichier</option>
                </select>
                @error('type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="formuler_id"
                    ...
                    @foreach($formulers as $formuler)
                        <option value="{{ $formuler->id }}" {{ old('formuler_id', $variable->formuler_id) == $formuler->id ? 'selected' : '' }}>
